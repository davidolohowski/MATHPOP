...
Y_obs <- read_csv('/project/6055726/dli346/GC_count_PP/data/prob_GC_data/V15ACS_pGC_Jans.csv')
...
c <- rbind(c(4106, 3168), c(1080, 1036), c(465, 3590), c(1637, 1645))/4300*76
colnames(c) <- c('x', 'y')
e <- c(1.6, 1.158, 0.74, 0.88)
theta <- c(3*pi/4, 5*pi/6, pi/6, 0)
UDG_ID <- c('W56', 'W59')

prior <- list(b0 = c(log(0.04), 0.4), 
                 N = data.frame(a = c(log(100), log(20), 0, 0), b = c(0.25, 0.25, rep(50, 2))), 
                 R_eff = data.frame(a = c(log(10), log(6), log(1.2), log(1)), b = c(0.25, 0.25, 0.5, 0.5)),
...
cf_error <- list(alpha = 6, m50 = 26.7, beta0 = 0.07477, beta1 = 0.75094, m1 = 25.5)

set.seed(seed[sim_id])
Theta <- list(b0 = runif(1, 0.03, 0.05), 
              N = matrix(c(rnorm(1, log(100), 0.25), rnorm(1, log(20), 0.25), rnorm(2, log(10), 0.5)), nrow = 1), 
              R_eff = matrix(c(rnorm(1, log(10), 0.25), rnorm(1, log(6), 0.25), rnorm(2, log(2), 0.5)), nrow = 1), 
...
# 40min - 50k
saveRDS(res_prob, paste0('/project/6055726/dli346/GC_count_PP/data/v15acs/res_prob_v15acs_Jans_seed_', seed[sim_id], '.RDS'))
