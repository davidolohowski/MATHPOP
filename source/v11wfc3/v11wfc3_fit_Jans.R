...
Y_obs <- read_csv('/project/6055726/dli346/GC_count_PP/data/prob_GC_data/V11WFC3_pGC_Jans.csv')
...
c <- rbind(c(2747, 3129), c(1389, 1058))/4400*62
colnames(c) <- c('x', 'y')
e <- c(1, 1/0.65)
theta <- c(0, -pi/4)
UDG_ID <- c('W80', 'W79')
...
prior <- list(b0 = c(log(0.03), 0.4), 
                 ...
                 R_eff = data.frame(a = c(log(1.5), log(2.22)), b = rep(0.5, 2)),
...
cf_error <- list(alpha = 6.7, m50 = 26.6, beta0 = 0.07477, beta1 = 0.75094, m1 = 25.5)
...
# 23min - 50k
saveRDS(res_prob, paste0('/project/6055726/dli346/GC_count_PP/data/v11wfc3/res_prob_v11wfc3_Jans_seed_', seed[sim_id], '.RDS'))
