...
Y_obs <- read_csv('/project/6055726/dli346/GC_count_PP/data/prob_GC_data/V10ACS_pGC_Jans.csv')
...
c <- rbind(c(3278, 4073), c(3727, 2607), c(3600, 610), c(536, 2744))/4300*76
colnames(c) <- c('x', 'y')
e <- c(1.4, 1.3, 1.17, 1.1)
theta <- c(pi/2, 0, 0, 0)
UDG_ID <- c('R20','R21','R89')

prior <- list(b0 = c(log(0.05), 0.4), 
                 N = data.frame(a = c(log(150), rep(0, 3)), b = c(0.25, rep(50, 3))), 
                 R_eff = data.frame(a = c(log(2.5), log(1.72), log(2), log(0.64)), b = c(0.25, 0.5, 0.5, 0.5)),
                 n = data.frame(a = log(c(0.5, rep(1, 3))), b = c(0.5, rep(0.75, 3))),
...
cf_error <- list(alpha = 5.5, m50 = 26.8, beta0 = 0.07477, beta1 = 0.75094, m1 = 25.5)

set.seed(seed[sim_id])
Theta <- list(b0 = runif(1, 0.04, 0.06), 
              N = matrix(c(rnorm(1, log(150), 0.25), rnorm(3, log(10), 0.5)), nrow = 1), 
              R_eff = matrix(c(rnorm(1, log(2.5), 0.25), rnorm(3, log(2), 0.5)), nrow = 1), 
              n = matrix(c(rnorm(1, log(1), 0.5), rnorm(3, log(1), 0.75)), nrow = 1),
              mu = matrix(rnorm(5, 26.2, 0.5), nrow = 1), 
              sigma = matrix(rnorm(5, log(1.2), 0.1), nrow = 1))

set.seed(seed[sim_id])
res_prob <- fit_mod_np_gal(S, grid, Theta, UDG_ID, Ng = 1, c, e, theta, Y_obs, p = 1, prior, M = 300000, tune, cf_error = cf_error)

saveRDS(res_prob, paste0('/project/6055726/dli346/GC_count_PP/data/v10acs/res_prob_v10acs_Jans_seed_', seed[sim_id], '.RDS'))
